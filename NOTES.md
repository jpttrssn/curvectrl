...
- Thumbs are rendered as inverted positives via the film pipeline: per-channel level normalize → crop masked borders → half-res true-color demosaic (gray box-average fallback) → collapse to linear Rec.709 luminance (`luma`; mono film carries no color signal, so this removes every capture cast outright — per-channel casts showed up as purple shadows on underexposed frames before this) → `measure_base` (p95 of the mono buffer = black point anchored on clear film) → `film::invert_gray` (density-space B&W inversion, always on, hardcoded stock) → linear-light area-average downscale to `THUMB_SIZE` → sRGB transfer curve. cpp≥3 sources pass through the same collapse+anchoring+inversion.
...
- Pipeline: `rawloader::decode_file` → `normalize_samples` (per-CFA-position black/white levels for Integer data; max-gain for Float) → `crop_samples` (slices off `crops`-flagged masked borders; skipped if degenerate) → bayer: `demosaic_half` on the CFA shifted by the crop origin (`cfa.shift(left, top)`; 2×2 blocks accumulated via `color_at`; `None` → legacy gray box-average); cpp≥3: passthrough → `luma()` (Rec.709 weights in linear light → mono f32 stride 1) → scalar `film::measure_base` + `.filter(>= MIN_PLAUSIBLE_BASE)` + `film::invert_gray(mono, &ACTIVE_STOCK, base)` → `resize_area(..., channels)` (block-average downscale to `THUMB_SIZE`, in linear light, before tone encoding — suppresses noise/grain aliasing; generic over channel count, currently called with 1) → `srgb_encode` on all values → u8 quantize as `[v,v,v,255]` → orientation fix (`orient`, all rawloader variants) → `Handle::from_rgba`. White balance was removed when inversion landed; luminance collapse later made even per-channel base anchoring unnecessary for mono stocks.
